...
class HigherReinforceModule(nn.Module):
    def __init__(self, rel_dim, hidden_dim):
        ...
        self.project_layer = nn.Sequential(nn.Linear(hidden_dim+rel_dim, 32), nn.ReLU(), nn.Linear(32, 2))
    ...
    def forward(self, sent_encoding, relation_idx):
        ...
        """
        state = torch.cat((sent_encoding, self.relation_vector[relation_idx]), dim=0)
        results = torch.softmax(self.project_layer(state), dim=0)
        return results
...
class LowerReinforceModule(nn.Module):
    def __init__(self, hidden_dim, ):
        ...
        self.project_layer = nn.Sequential(nn.Linear(hidden_dim, 16), nn.ReLU(), nn.Linear(16, 2))
    ...
    def forward(self, sent_encoding):
        ...
        """
        return torch.softmax(self.project_layer(sent_encoding), dim=0)
